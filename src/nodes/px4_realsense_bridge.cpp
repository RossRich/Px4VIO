...
PX4_Realsense_Bridge::PX4_Realsense_Bridge(const ros::NodeHandle &nh) : nh_(nh) {
  ...
  odom_sub_ = nh_.subscribe<const nav_msgs::Odometry &>(inputTopic.c_str(), 50, &PX4_Realsense_Bridge::odomCallback, this);
  // publishers
  mavros_odom_pub_ = nh_.advertise<nav_msgs::Odometry>("mavros/odometry/out", 50);
  mavros_system_status_pub_ = nh_.advertise<mavros_msgs::CompanionProcessStatus>("mavros/companion_process/status", 10);
